#|
Due MIDNIGHT the evening of FRIDAY, NOVEMBER  1.

As usual, mail the TA your implementation of this file, and a good project report detailing experiments you
did changing various parameters (initial weights, alpha, hidden units, etc., etc.)
and their effects on success or failure to converge to a solution.  See the questions at the end
of the file.  Note that there is one "extra credit" implementation function.

This project isn't particularly difficult to implement, though it may be difficult
to debug because of the stochasticity of the system.  This means that the report means
a lot more than the code implementation this time.  I want to see a serious collection
of experiments analyzing and comparing parameter settings and their outcomes.  Keep in
mind that this system is STOCHASTIC, just like evolutionary computation, so you'll need
to follow similar rules in your experiments as you did in that project.

 I'd also be interested in modifications you made to the
project to add interesting gizmos.  For example, the project doesn't have bias units
in the hidden nodes.  Another interesting item is to find additional problems.
You might dig up some interesting problems at the UCI Machine Learning Repository
(google for it).  If you're iterested in this route I can suggest some problems.
...
As in:  C = A . B
no dot          means element-by-element multiply -- by which we mean if C = (A) (B) , then for each index i and j
  Cij = Aij * Bij
...
1. SIGMOID		done
2. NET-ERROR 	        done
3. FORWARD-PROPAGATE    done
4. back-propagate	done
5. NET-BUILD		done, I think
6. SIMPLE-GENERALIZATION
...
I have built in a bias unit in my conversion function so you can forget about
that issue.  There's no bias unit in the hidden nodes though.

For the NAND and XOR problems, you have to get the code working.  Each usually requires
...
If you're interested in doing more interesting stuff, you could try:
...
problems, including tweaks you added and lessons learned. But I strongly
urge you to do a lot more than this.  Figures, analysis, etc.
If you wanted to really do this right, the last chapter of Esentials of 
Metaheuristics discusses how to do difference of means testing.
...
(defun make-random-matrix (i j val)
  ...
  (make-matrix i j #'(lambda (x)
		       ...
		       (- (random (* 2.0 val)) val))))
#|(defun printamatrix ()
  (setf abcd (make-random-matrix 3 3 2))
  abcd
)|#
(defun e (matrix i j)
...
(defun net-error (output correct-output)
  ...
  (scalar-multiply .5 (multiply (transpose (subtract correct-output output)) (subtract correct-output output)))
)
#|
(defun test-error ()
  (setf vec-x (transpose (make-random-matrix 1 3 2)))
  (setf vec-y (scalar-add .1 vec-x))
  (net-error vec-y vec-x)
)|#
...
the datum as input."
  ;;(let ((col-datum (convert-data datum))))
  (let* (
          ;(datum (convert-data (list datum)))
          (h (map-m #'sigmoid (multiply v (first (first datum)))))
...
returning a list consisting of new, modified V and W matrices."
  ...
  (let* (
          ;(datum (convert-data (list datum)))
          ;;forward propagation to get vector o:
          (h (map-m #'sigmoid (multiply v (first (first datum)))))
          (o (map-m #'sigmoid (multiply w h)))
          ;back propagation:
          (c (second (first datum)));;the second list in datum is the expected output. Is in list form
          ...
          (v (add v (scalar-multiply alpha (multiply hdelta (transpose (first (first datum)))))))
        )
        (append (list v) (list w));return modified v and w as a list
    )
)
...
and the final W matrix of the learned network."
  ...
  (setf v (make-random-matrix num-hidden-units (list-length (first (first data))) initial-bounds));#hidden by #inputs
  (setf w (make-random-matrix (list-length (second (first data))) num-hidden-units initial-bounds));#out by #hidden
  (setf worsterr 0)
  (setf listoferrs ())
  (setf iter 0)
  (setf condmet nil)

  (while (and (< iter max-iterations) (not condmet));might be off-by-one or need a do-while here
    
    ;1. do back propagation...
    ;(if (eq (mod iter (list-length data)) 0)
    (setf datum (list (elt data (mod iter (list-length data)))))
      
    (setf back-result (back-propagate datum alpha v w));backpropagate with current datum using alpha, v, and w
    (setf v (first back-result))
    (setf w (second back-result))
    
    (if (eq (mod iter modulo) 0);2. every modulo iterations...
        (progn
         (loop for i in data do ;For every data element in the data, perform forward propagation...
            
            (setf calculated-output (forward-propagate (list i) v w));i is the current datum, v and w are the neural network weights from back-propagation
            ;(append listoferrs calculated-output)
            (setf listoferrs (flatten (cons (net-error calculated-output (second i)) listoferrs)))
            
          )
          (if (eq print-all-errors t)
              (print (reverse listoferrs))
          )
          (setf worsterr (maxerror listoferrs))
          ;;print worst err and mean err
          (print "worst error: ")
          (print (maxerror listoferrs))
          (print "average: ")
          (print (average listoferrs))
          
          (if (< worsterr *a-good-minimum-error*)
            (progn
              (print "Worst error looks pretty good. Now exiting.")
              (setf condmet t)
              (return-from net-build (append (list v) (list w)))
            )
          )
          (setf listoferrs ())
          (setf worsterr 0)
  ...
  (append (list v) (list w));might need to wrap v and w into separate lists.
)
;(net-build (convert-data *nand*) 3 1.0 5 20000 1000 t) ;initial bound of +-5, 20k max iterations, mod is 1000
...
and use a modulo of MAX-ITERATIONS."
  (setf training-data (convert-data data))
  #|(loop for i from 0 to (/ (list-length training-data) 2) do
    (print i)
    (print (elt training-data i))
  )|#
  (if (oddp (list-length training-data));figure out the bounds
    (progn
      (setf first-half-length (/ (- (list-length training-data) 1) 2));if odd number of data points
      (setf generalizing-length (- (list-length training-data) first-half-length))
    )
    (progn
      (setf first-half-length (/ (list-length training-data) 2));else divides evenly
      (setf generalizing-length (- (list-length training-data) first-half-length))
    )
  )
  
  (setf training-half (subseq training-data 0 first-half-length));training-half is the data from first half
  (setf generalizing-data (subseq training-data first-half-length (list-length training-data)));generalizing-half is the second half of the data
  ;(print generalizing-half)
  (setf neuralweights (generalize-helper training-half num-hidden-units alpha initial-bounds max-iterations max-iterations))
  (setf generalv (first neuralweights))
  (setf generalw (second neuralweights))
  ;(print generalv)
  ;(print generalw)
  (setf forwardouts ())
  ;(loop for i in generalizing-data do ;For every data element in the data, perform forward propagation...
  ;  (setf calculated-output (forward-propagate (list i) generalv generalw));i is the current datum, v and w are the neural network weights
    ;(append listoferrs calculated-output)
    ;(setf listoferrs (flatten (cons (net-error calculated-output (second i)) listoferrs)))
 ; )
  ;(print forwardouts)
  ;(generalize-average generalizing-data num-hidden-units alpha initial-bounds max-iterations max-iterations)
  (generalize-average generalizing-data generalv generalw)
)



(defun generalize-helper (data num-hidden-units alpha initial-bounds max-iterations modulo);this is basically net-build, but it doesn't print anything!
  (setf v2 (make-random-matrix num-hidden-units (list-length (first (first data))) initial-bounds));#hidden by #inputs
  (setf w2 (make-random-matrix (list-length (second (first data))) num-hidden-units initial-bounds));#out by #hidden
  (setf worsterr 0)
  (setf listoferrs ())
  (setf iter 0)
  (setf condmet nil)
  ...
  (while (and (< iter max-iterations) (not condmet));might be off-by-one or need a do-while here
    ...
    ;(if (eq (mod iter (list-length data)) 0)
    (setf datum (list (elt data (mod iter (list-length data)))))
    (setf back-result (back-propagate datum alpha v2 w2));backpropagate with current datum using alpha, v, and w
    (setf v2 (first back-result))
    (setf w2 (second back-result))    
    (if (eq (mod iter modulo) 0);2. every modulo iterations...
        (progn
         (loop for i in data do ;For every data element in the data, perform forward propagation...
            (setf calculated-output (forward-propagate (list i) v2 w2));i is the current datum, v and w are the neural network weights from back-propagation
            (setf listoferrs (flatten (cons (net-error calculated-output (second i)) listoferrs)))
         )
          (setf worsterr (maxerror listoferrs))
          (if (< worsterr *a-good-minimum-error*)
            (progn
              (setf condmet t)
              (return-from generalize-helper (append v2 w2))
            )
          )
          (setf listoferrs ())
          (setf worsterr 0)
...
(defun generalize-average (data v w)
  (setf listoferrs ())
  (loop for i in data do ;For every data element in the data, perform forward propagation...
    (setf calculated-output (forward-propagate (list i) v2 w2));i is the current datum, v and w are the neural network weights from back-propagation
    (setf listoferrs (flatten (cons (net-error calculated-output (second i)) listoferrs)))
  ...
  (setf worsterr (maxerror listoferrs))
  (if (< worsterr *a-good-minimum-error*)
    (return-from generalize-average (average listoferrs))
  )
  ;(setf listoferrs ())
  ;(setf worsterr 0)
  (setf iter (+ 1 iter))
...
(defun k-fold-validation (data k)
...
#|
 (data num-hidden-units alpha initial-bounds max-iterations modulo &optional print-all-errors)
...
;(net-build (convert-data *voting-records*) 4 1.5 4 30000 250 t)
